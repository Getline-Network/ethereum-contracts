...
contract TestInvestorLedger {
    ...
    function createMockToken(string memory tokenName)  returns (PrintableToken) {
        ...
        return new PrintableToken(tokenName, decimalPoints, tokenSymbol, PRINT_VALUE);
    }
    ...
    function testGatherCollateral() {
        ...
        Assert.equal(collateralToken.balanceOf(borrower), PRINT_VALUE, "It should print some tokens");
        ...
        Assert.equal(testLedger.totalCollateral, PRINT_VALUE, "It should add collateral tokens");
    }
    function testGatherInvestment() {
        ...
        uint256 totalCollateral = PRINT_VALUE;
    ...
    /* function testReleaseLoanToBorrower() {
        ...
        Assert.equal(testLedger.loanWidthdrawn, expectedFalse, "By default loan is not withdrawn");
